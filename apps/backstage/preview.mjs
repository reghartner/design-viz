...
const generated=await readFile(path.join(app,'src/generated/viewerDocument.ts'),'utf8');
const hash=JSON.parse(generated.match(/export const viewerScriptCsp = (.*);/)[1]);
await writeFile(path.join(out,'csp.txt'),`default-src 'self'; script-src 'self' ${hash}; style-src 'self' 'unsafe-inline'; font-src data:; img-src 'self' data:; connect-src 'self'; frame-src 'self'; object-src 'none'; base-uri 'self';`);
