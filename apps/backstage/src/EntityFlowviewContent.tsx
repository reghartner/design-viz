...
export function EntityFlowviewContent(){
  ...
  const specLoader=useMemo(()=>createSpecLoader(discovery,fetchApi,proxyPath),[discovery,fetchApi,proxyPath]);
  return <FlowviewEntityDiagrams entityRef={stringifyEntityRef(entity).toLowerCase()} loadDiagrams={loader} loadSpec={specLoader}/>;
}
