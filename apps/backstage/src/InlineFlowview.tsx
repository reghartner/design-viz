...
export function InlineFlowview({diagram,loadSpec,target}:{diagram:AssociatedDiagram;loadSpec:SpecLoader;target?:ViewerTarget}){
  ...
  useEffect(()=>{
    const abort=new AbortController();disconnect();setState({loading:true});setRenderError('');setRendered(false);setHeight(640);
    loadSpec({id:diagram.id,revision:diagram.revision},abort.signal).then(spec=>{
      if(!abort.signal.aborted)setState({spec,loading:false});
    },error=>{if(!abort.signal.aborted)setState({loading:false,error:error instanceof Error?error.message:'Unable to load diagram.'});});
    return ()=>{abort.abort();disconnect();};
  },[diagram.id,diagram.revision,loadSpec,attempt,disconnect]);
  const connect=useCallback(()=>{
    disconnect();if(!iframe.current?.contentWindow || !state.spec)return;
    const next=new MessageChannel();channel.current=next;
    timer.current=setTimeout(()=>setRenderError('The embedded viewer did not start. Check the Backstage frame/script content-security policy, then retry.'),10000);
    next.port1.onmessage=event=>{
      if(channel.current!==next)return;
      const message=event.data;
      if(message?.type==='size' && Number.isFinite(message.height))setHeight(Math.max(480,Math.min(24000,Math.ceil(message.height))));
      if(message?.type==='rendered'){clearTimeout(timer.current);setRendered(true);setRenderError('');}
      if(message?.type==='error' || message?.type==='navigation-error'){
        clearTimeout(timer.current);setRenderError(typeof message.message==='string'?message.message:'Unable to render diagram.');
      }
    ...
    iframe.current.contentWindow.postMessage({type:'flowview:init',spec:state.spec,target:targetRef.current},'*',[next.port2]);
  },[state.spec,disconnect]);
  useEffect(()=>{if(target && rendered)channel.current?.port1.postMessage({type:'navigate',target});},[target,rendered]);
  useEffect(()=>{
    const pause=()=>channel.current?.port1.postMessage({type:'pause'});
    const visibility=()=>{if(document.hidden)pause();};
    document.addEventListener('visibilitychange',visibility);
    const observer=typeof IntersectionObserver==='undefined'?null:new IntersectionObserver(entries=>{if(entries.some(entry=>!entry.isIntersecting))pause();});
  ...
  return <div aria-label="Inline diagram viewer">
    ...
    {state.spec!==undefined && <>
      ...
      <iframe ref={iframe} key={attempt} title={'Flowview: '+diagram.title} srcDoc={viewerDocument} onLoad={connect}
        ...
        style={{display:'block',width:'100%',height,border:0,borderRadius:8,background:'transparent'}}/>
    </>}
  </div>;
}
