...
| 8 | Retry / timeout / circuit-breaker view | Attempts, deadlines, jitter windows, retry storms, open/half-open recovery, store-and-forward | Starter with offset waterfall + tables + budget + checks + state; success, admission denial and circuit recovery, plus failed-probe recipe | M | Built, authored scenarios; storm/fleet comparison remains |
| 9 | Deployment and trust boundaries | Regions, AZs, tenant isolation, VPCs, edge/cloud ownership, secure boot partitions | Semantic nested containers with relationship types; distinct deployment view instead of forcing the row-based flow layout | L | Planned |
| 10 | Replication and consistency panel | Leader/follower positions, quorum, stale reads, conflicts, CRDT or offline reconciliation | `replicas`: same-sequence position ruler, explicit reference, reported lag, availability, observation labels; checks supply read/commit outcomes | M | Built, numeric cursor slice; vector clocks and consensus simulation deferred |
...
| 12 | Canary and migration rollout | Cohorts, traffic split, schema compatibility, dual writes, rollback conditions; firmware fleets | Cohort tiles + checks + traffic shares; separate release version from runtime health | M | Canary/firmware starter built; schema migration and dual-write stories remain |
