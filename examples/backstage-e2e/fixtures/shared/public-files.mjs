...
export async function readPublicFile(root, relative, publicPaths) {
  if (typeof relative !== 'string' || /[\\\x00-\x1f\x7f]/.test(relative)) return null;
  const segments = relative.split('/');
  if (segments.some(segment => !segment || segment === '.' || segment === '..')) return null;
  const selected = publicPaths.find(prefix => relative === prefix || relative.startsWith(prefix + '/'));
  if (!selected) return null;
  try {
    const canonicalRoot = await realpath(root);
    const boundary = path.join(canonicalRoot, selected);
    const filename = await realpath(path.join(canonicalRoot, relative));
    if (filename !== boundary && !filename.startsWith(boundary + path.sep)) return null;
