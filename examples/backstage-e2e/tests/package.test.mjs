...
const { scan } = await import(
  pathToFileURL(path.join(root, 'tools/canon/drift.mjs'))
);
const { LocalGitSources } = await import(
  pathToFileURL(path.join(root, 'tools/canon/local-git.mjs'))
);
...
async function checkPublicFileBoundaries(
  directory,
  modulePath,
  createName,
  prefix,
  ...
  const { [createName]: createServer } = await import(
    pathToFileURL(path.join(directory, modulePath))
  );
  ...
  try {
    ...
    for (const suffix of [
      '..%2f.local/review-canary.txt',
      '%2e%2e%2f.local/review-canary.txt',
      'review-shortcut.txt',
    ]) {
    ...
    assert.equal(
      (await rawGet(port, validPath)).status,
      200,
      'Malformed URLs must not crash the server'
    );
  } finally {
...
test('repository inputs cannot become paths, shell fragments, or ambiguous identities', () => {
  assert.equal(
    options(['--out', '/tmp/new demo', '--owner', 'fictional-home']).mockRepo,
    'backstage-designer-mock'
  );
  for (const args of [
...
test(
  'fresh portable repositories seed over HTTP, pin real code, and detect both experiments',
  { timeout: 180000 },
  async (t) => {
    const temporary = await mkdtemp(
      path.join(tmpdir(), 'flowview-public-package-')
    );
    t.after(() => rm(temporary, { recursive: true, force: true }));
    // Spaces in the destination and dots in repo names exercise argument/URL handling.
    const out = path.join(temporary, 'new company'),
      mock = path.join(out, 'sample.mock'),
      designer = path.join(out, 'sample.designer');
    const args = [
      'examples/backstage-e2e/create.mjs',
      '--out',
      out,
      '--owner',
      'fixture-company',
    ...
    const catalog = JSON.parse(
      await readFile(path.join(designer, 'workbench/catalog.json'))
    );
    assert.equal(catalog.services.length, 3);
    assert.equal(
      catalog.services.flatMap((s) => s.apis).flatMap((a) => a.operations)
        .length,
      4
    );
    const spec = JSON.parse(
      await readFile(path.join(designer, 'specs/doorbell.json'))
    );
    ...
    const baseline = (
      await run('git', ['rev-parse', 'main'], mock)
    ).stdout.trim();
    for (const ref of references) {
      assert.equal(
        ref.repository,
        'https://github.com/fixture-company/sample.mock'
      );
      assert.equal(ref.revision, baseline);
      const source = (
        await run('git', ['show', ref.revision + ':' + ref.path], mock)
      ).stdout;
    ...
    const runtime = JSON.parse(
      await readFile(path.join(designer, '.flowview/runtime.json'))
    );
    assert.equal(
      runtime.revision,
    ...
    await checkPublicFileBoundaries(
      mock,
      'mock/server.mjs',
      'createMockServer',
      '/files/catalog/',
    ...
    await checkPublicFileBoundaries(
      designer,
      'server/server.mjs',
      'createDesignerServer',
      '/workbench/',
      'workbench',
      '/workbench/catalog.json'
    );
    // Valid mixed-case IDs must work through both association and revision reads.
    const { createDesignerServer } = await import(
      pathToFileURL(path.join(designer, 'server/server.mjs'))
    );
    const specFile = path.join(designer, 'specs/doorbell.json'),
      originalSpec = await readFile(specFile, 'utf8');
    const registryFile = path.join(designer, 'registry.json'),
      originalRegistry = await readFile(registryFile, 'utf8');
    const mixedRegistry = JSON.parse(originalRegistry);
    mixedRegistry.diagrams[0].id = 'Doorbell';
    const mixed = structuredClone(spec);
    mixed.page.canon.id = 'Doorbell';
    ...
    try {
      ...
      const indexed = await (
        await fetch(
          base +
            '/api/canon/entity-diagrams?entityRef=component:home/recording-service',
          { headers }
        )
      ).json();
      ...
      const published = await fetch(
        base +
          '/api/canon/specs/Doorbell?revision=' +
          indexed.diagrams[0].revision,
        { headers }
      );
    ...
    const sources = new LocalGitSources({
      'https://github.com/fixture-company/sample.mock': mock,
    });
    ...
    await assert.rejects(
      run(process.execPath, ['--test', 'contract.test.mjs'], mock),
      (error) => error.code === 1
    );
    ...
    assert.equal(
      references[1].revision,
      baseline,
      'No experiment auto-accepts a new baseline'
    );
    await run('git', ['switch', 'main'], mock);
    assert.equal((await scan([spec], sources)).findings.length, 0);
    for (const directory of [mock, designer]) {
      assert.equal(
        (await run('git', ['status', '--porcelain'], directory)).stdout.trim(),
        ''
      );
      ...
      assert.equal(
        (await run('git', ['remote'], directory)).stdout.trim(),
        '',
        'Local setup never publishes'
      );
    }
    assert.equal(
      (
    ...
    const workflows =
      ...
      (await readFile(
        path.join(designer, '.github/workflows/drift.yml'),
        'utf8'
      ));
    assert.match(workflows, /repository: fixture-company\/sample\.mock/);
    assert.doesNotMatch(
      workflows,
      /reghartner\/backstage-designer|__MOCK_|auto-merge|gh pr merge/
    );
    const before = await readFile(
      path.join(designer, 'specs/doorbell.json'),
      'utf8'
    );
    await assert.rejects(run(process.execPath, args, root), /EEXIST/);
    assert.equal(
      await readFile(path.join(designer, 'specs/doorbell.json'), 'utf8'),
      before,
