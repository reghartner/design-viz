...
function initWorkbenchBuilder(opts){
  ...
  view.addEventListener('click', function(ev){
    ...
    if (ev.target.closest && ev.target.closest('.tabbtn'))
      setTimeout(injectZoomControls, 0); /* newly shown boards can now measure */
  ...
  function applyBoardZoom(secEl){
    ...
    /* measure the STOCK 100% width first (inline styles cleared), then
       scale in absolute pixels — percentage widths track the canvas, so
       on canvases narrower than the core sheet's min-width floor a
       percentage "zoom in" could actually shrink the svg */
    svg.style.width = '';
    svg.style.minWidth = '';
    canvas.classList.remove('dv-zoomed');
    if (scale !== 1){
      var base = svg.getBoundingClientRect().width;
      if (base > 0){ /* a board in a hidden tab measures 0 — leave stock */
        svg.style.minWidth = '0';
        svg.style.width = (base * scale) + 'px';
