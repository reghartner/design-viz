    ...
    Object.keys(s.diagram && s.diagram.nodes || {}).forEach(function(id){
      ...
      if(detail.ports!=null){
        if(!specObject(detail.ports))errors.push(where+'.ports: expected {in, out}');
          ...
          else if(!(child.rows || []).some(function(row){return row.some(function(slot){return (Array.isArray(slot)?slot:[slot]).indexOf(port)>=0;});}) && !(child.floats || []).some(function(f){return f.id===port;}))
