...
/* A re-render (workbench Render click, skin switch) rebuilds the DOM and so
   ...
   place across renderPage calls: capture the active tab's canonical
   reference per tabs block (the unique label slug, or the deep-link
   positional form when labels collide — the same grammar the URL hash
   uses), then re-select whatever it resolves to in the new render. Blocks
   are matched by position; a reference with no match in the new block keeps
   the render default (first tab). */
function activeTabReferences(ctl){
  if (!ctl || !ctl.tabBlocks || !ctl.tabBlocks.length) return null;
  return ctl.tabBlocks.map(function(tb){ return tabReference(tb.slugs, tb.active()); });
}
function restoreActiveTabs(ctl, saved){
  if (!ctl || !ctl.tabBlocks || !saved) return;
  ctl.tabBlocks.forEach(function(tb, i){
    var ref = i < saved.length ? saved[i] : null;
    var idx = ref != null ? tabIndexOf(ref, tb.slugs, tb.count) : -1;
