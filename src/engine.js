...
function edgePath(e, L, adj){
  ...
  if (a.float || b.float){
    ...
    if (b.float){
      ...
      var ex = b.cx > a.cx ? b.cx - b.w/2 : b.cx + b.w/2, ey = b.cy + (bBelow ? -b.h/2 : b.h/2);
      return 'M ' + sx + ' ' + sy + ' C ' + (sx + (ex-sx)*0.25 + avX) + ' ' + (sy + (ey-sy)*0.5) + ' ' +
             (ex - (ex-sx)*0.3 + avX) + ' ' + (ey - (ey-sy)*0.35) + ' ' + ex + ' ' + ey;
    }
...
function resolveEdgeAvoidance(edges, L, adj){
  edges.forEach(function(e, ei){
    ...
    var bestVal = 0, bestHits = base;
    for (var ci = 0; ci < cands.length; ci++){
      adj[ei][key] = cands[ci];
      var h = countPathRectHits(samplePathD(edgePath(e, L, adj[ei])), rects);
      if (h < bestHits){ bestHits = h; bestVal = cands[ci]; }
      if (h === 0) break;
    }
    adj[ei][key] = bestVal;
  });
