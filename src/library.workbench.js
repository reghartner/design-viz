...
function initWorkbenchLibrary(opts){
  ...
  var pending=null,entries=[],origin='',ctl=null,active=null,sequence=0,current=null;
  function stop(){
    sequence++;active=null;current=null;edit.disabled=true;
    if(ctl)ctl.destroy();ctl=null;reader.replaceChildren();
  }
  function load(){
    if(pending)return pending;
    pending=(async function(){
      var raw=opts.builtin;
      ...
      if(location.protocol!=='file:'){
        ...
        if(response.status!==404){
          ...
          raw=JSON.parse(text);origin='Published repository snapshot';
        }
  ...
  function cards(){
    ...
    entries.forEach(function(entry){
      var button=document.createElement('button');button.type='button';button.className='canon-library-card';
      ...
      button.addEventListener('click',function(){opts.open(entry.id);});grid.appendChild(button);
    });
  }
  async function show(screen,id){
    ...
    try{
      ...
      ctl=renderPage(reader,normalize(spec),spec.page.skin,null,{autoplay:false});edit.disabled=false;
    }catch(ex){
