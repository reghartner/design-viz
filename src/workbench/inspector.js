...
function createBuilderInspector(opts){
  ...
  function captureTextareaSizes(){
    ...
    Array.prototype.forEach.call(guide.querySelectorAll('textarea'),function(control){
      var key=control.getAttribute('data-inspector-control');
      // Native resizing writes inline dimensions. Leave CSS-sized controls fluid.
      if(key && (control.style.height || control.style.width))sizes[key]={height:control.style.height,width:control.style.width};
    });
    return sizes;
  }
  function restoreTextareaSizes(sizes){
    if(!sizes)return;
    Array.prototype.forEach.call(guide.querySelectorAll('textarea'),function(control){
      var size=sizes[control.getAttribute('data-inspector-control')];
      if(size){control.style.height=size.height;control.style.width=size.width;}
    });
  }
