...
test('domain example validates and uses stable references for nested detail and index traversal',()=>{
 const p=copy().page,findings=plain(B.validate(p));assert.deepEqual(findings.errors,[]);assert.equal(findings.warnings.length,1);assert.match(findings.warnings[0],/inline expansion was removed/);
...
test('detail validation rejects missing targets, unsafe URLs, ambiguous IDs and invalid boundary/step mappings',()=>{
 for(const mutate of [p=>p.sections[0].diagram.nodes.connectivity.detail.section='missing',p=>p.sections[0].diagram.nodes.connectivity.detail.url='javascript:alert(1)',p=>p.sections[1].id=p.sections[0].id,p=>p.sections[0].diagram.nodes.connectivity.detail.ports.in='missing',p=>p.sections[0].diagram.nodes.connectivity.detail.stepMap.nonexistent={step:'missing'}]){
  const p=copy().page;mutate(p);assert.ok(B.validate(p).errors.length,JSON.stringify(p));
 }
});
test('legacy expansion specs remain readable without ports or source migration',()=>{
 const p=copy().page;delete p.sections[0].diagram.nodes.recording.detail.ports;
 const before=JSON.stringify(p),findings=B.validate(p);
 assert.deepEqual(plain(findings.errors),[]);assert.match(findings.warnings.join('\n'),/opens as a focused drilldown/);
 assert.equal(JSON.stringify(p),before);
 p.sections[0].diagram.nodes.recording.detail.mode='focus';
 assert.deepEqual(plain(B.validate(p).warnings),[]);
 p.sections[0].diagram.nodes.recording.detail.mode='link';
 assert.deepEqual(plain(B.validate(p).errors),[]);
 assert.match(B.validate(p).warnings.join('\n'),/use focus for local sections/);
});
test('legacy boundary references still validate their child placement',()=>{
