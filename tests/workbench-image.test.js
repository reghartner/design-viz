'use strict';
/* The normal Node CI job exercises the shipped image, including auto-discovery.
...
test('nginx image discovers a saved canon document and replaces a stale library',{skip:!dockerAvailable && !process.env.CI,timeout:240000},async t=>{
  ...
  spec.page.canon.id='ci-auto-library';
  const source=path.join(directory,'docs/diagrams/feature/story.spec.json');
  ...
  assert.deepEqual(library.diagrams.map(d=>d.spec),[spec]);
