...
function deviceAppPanelHTML(panel, state, fresh, notificationFresh, screenFresh) {
  ...
  var h =
    '<div class="deviceapp'+(m.showSources?'':' da-standalone')+'"><div class="da-phone da-phone-'+m.screen+'" data-da-screen="'+m.screen+'">'+
  ...
  h+='</div><div class="da-home" aria-hidden="true"></div></div>';
...
PanelRegistry.extend('deviceapp', {
  styles: [
    ...
    {
      order: 377,
      css: String.raw`.deviceapp.da-standalone{grid-template-columns:minmax(0,330px);max-width:354px;gap:0;justify-content:center;}
.da-phone{width:100%;max-width:330px;min-width:0;min-height:0;aspect-ratio:9/18.5;align-self:start;position:relative;box-sizing:border-box;display:flex;flex-direction:column;border:4px solid #79869e;border-radius:38px;padding:18px 12px 27px;background:#f5f7fc;color:#24324b;box-shadow:0 14px 30px #23324b18,inset 0 0 0 1px #ffffffa8;overflow:hidden;}
.da-phone::before{content:"";position:absolute;top:12px;left:calc(50% - 23px);width:46px;height:9px;border-radius:8px;background:#42506a;}
.da-statusbar{display:flex;flex:none;justify-content:space-between;gap:55px;font:600 10px 'IBM Plex Mono',monospace;padding:0 5px 18px;}
.da-statusbar>span:first-child{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.da-screen{flex:1;min-height:0;overflow:auto;overscroll-behavior:contain;scrollbar-width:thin;border-radius:10px;padding:2px 3px 8px;}
.da-screen:focus-visible{outline:2px solid #6875ca;outline-offset:-2px;}
.da-appbar{display:flex;align-items:center;gap:7px;font:600 11px/1.4 'Sora',sans-serif;color:#6673c2;padding:3px 4px 8px;}
.da-appbar svg{width:20px;height:20px;flex:none;}
...
.da-home-clock{padding:24px 0 22px;text-align:center;color:#384b71;}
.da-home-clock>span{display:block;font:600 10px/1.5 'Sora',sans-serif;letter-spacing:.12em;text-transform:uppercase;}
.da-home-clock strong{display:block;font:500 clamp(28px,10cqi,54px)/1.2 'Sora',sans-serif;letter-spacing:-.06em;margin-top:5px;overflow-wrap:anywhere;}
.da-home-empty,.da-empty-cards{padding:20px 12px;text-align:center;font:12px/1.5 'Sora',sans-serif;color:#65728a;}
.da-launcher{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;padding:16px 7px 8px;margin-top:auto;flex:none;}
.da-launcher-app{min-width:0;text-align:center;font:500 9px/1.35 'Sora',sans-serif;color:#344762;}
.da-launcher-icon{position:relative;display:grid;place-items:center;width:min(100%,47px);aspect-ratio:1;margin:0 auto 6px;border-radius:13px;background:#ffffffa8;box-shadow:0 4px 9px #536f8d13;color:#627494;}
.da-launcher-icon svg{width:25px;height:25px;}
.da-launcher-primary .da-launcher-icon{background:linear-gradient(145deg,#8292ec,#5564c2);color:#fff;}
.da-launcher-icon i{position:absolute;top:-5px;right:-5px;min-width:16px;padding:2px 4px;border-radius:10px;background:#cc4d6c;color:#fff;font:700 9px/1.4 'IBM Plex Mono',monospace;font-style:normal;}
...
@keyframes da-open-app{from{opacity:0;transform:translateY(24px) scale(.9);}to{opacity:1;transform:none;}}
